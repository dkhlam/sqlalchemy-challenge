...
def home():
    return(
        f"<center><h2>Welcome to the Hawaii Climate Analysis Local API!"
        f"<center><h3>Se;ect from one of the available routes:</h3></center>"
...
def dateStats(start=None, end=None):
    ...
    if not end:
        ...
        results = session.query(*selection)\
            ...
            .filter(Measurement.date <= endDate).all().all()
